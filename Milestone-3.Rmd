...
# If a value in a given column of a dataframe is not NOT a value or NA, it gets recoded as 1, otherwise, it becomes 0. 
...
# Creates the variable names by pasting a Q before a number, given by 1, 3, 5, and then a sequence by twos from 8 to 22. 
...
# .name_dummy is defined as a variable name in which "V" has been pasted before the number, then RL. For those columns, the recode_dummy is run on it to make it have the value of 1. 1 will signify Russian Law.
...
# .name_dummy is defined as a variable name in which "V" has been pasted before the number, then S. For those columns, the recode_dummy is run on it to make it have the value of 2. 2 will signify Sharia.
...
# .name_dummy is defined as a variable name in which "V" has been pasted before the number, then A. For those columns, the recode_dummy is run on it to make it have the value of 3. 3 will signify Adat.
...
# Lazarev creates a new column, called indexRL (in a non-tidyverse manner). indexRL is created by adding together all of the previously-created variables and dividing them by 10 (the number that would be created, given that the for loop was meant to be from 1:10).
...
# I'm not going to push the updated wp_data_survey.csv to git, because if that is the version that whoever is reading my code starts with, there will be no way for them to read the above chunk -- everything will be moot. It can only be the original version in github, I think. 
```
...
# The above code finally uses the multiplot() function that Lazarev created in the code chunk above all of these plots. One thing that is interesting to note is that my version of r does not appreciate him labelling the base_family as Helvetica in the theme_few() function. Not sure why this is, but my graphs look significantly uglier than his due to this. 
```
...
# Starts on page 685. Reads in survey data (original or updated? I'm personally uncertain; will require some messing around). **OVERALL: WOULD LIKE TO DISCUSS THIS SECTION, SOME THINGS I'M NOT SURE I UNDERSTAND. 
...
# qt() is the quantile function. You can use it to find out what the t-score is of the pth quantile (in this case, the 97.5th quantile). df stands for degress of freedom. In this case, it is equal to the length minus 1. This is typical, as 1 is the number of parameters we need, and the typical equation is the length minus number of parameters. He then multiplies it by the standard error. To be honest, I have no idea what's going on here. If qt() gave was just the equation for t-score, then maybe he would be multiplying by the standard error to cancel out the standard error in the denominator to just get the X - population mean number. But that doesn't make sense to me and doesn't look right according to the numbers in the resulting table. When I divided error1 by the standard error, I got 2 for each, every time. This makes sense, given that the error number is always 2x the se, but I do not know what this means/why we need it. Interestingly, it gets used later as the determinant for the error bar in the graph...is it a more useful version of standard error? 
...
# Again, very much irritates me that he writes over his code and then reads it in again. I need to figure out if this is the original survey data, or the edited surey data. A message is even thrown due to this issue when I read it in now. 
...
# I'm going to have to change the font on all of these because Helvetica does not run in my version of R. 
